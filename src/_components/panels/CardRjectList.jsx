...
import { deleteExpensepanelAction, expensetableTrueSubmit, exportExpenseListAction, getManagerListAction, getexpensePanelListAction, managercardRejectAction, managercardapprovelAction } from "../../store/Actions/Actions";
...
const CardRejectList = () => {
  ...
  useEffect(() => {
    if (managerRejectSelector) {
      const allExpense = managerRejectSelector.results.map((element,index) => ({
        srno:index+1,
        id: element.id,
