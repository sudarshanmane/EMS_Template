...
import { getAllUser, addAllUser } from "../../store/Action/Actions";

const AllUserPanel = () => {
  const [url, setUrl] = useState(URLS.GET_ALL_USER_URL);
  ...
  const createAllUserSelector = useSelector(
    (state) => state.createAllUserSuccess
  );

  useEffect(() => {
    if (createAllUserSelector && submittedValues) {
      const createAllUser = createAllUserSelector;
      if (createAllUser) {
        dispatch(getAllUser({ payload: {}, URL: url }));
        setSubmittedValues(null);
      }
    }
  }, [createAllUserSelector, submittedValues]);
  ...
  const getAllUserSelector = useSelector((state) => state.getTravelSuccess);
  useEffect(() => {
    if (getAllUserSelector) {
      const allUser = getAllUserSelector.map((element) => {
        return {
          id: element.id,
        };
      });
