...
import { userRegister } from "../store/Action/Actions";
import { setUser } from "../utils/sessionStorage";
// import { getdeptDropdownList } from "../../../store/department";
// import { getdesigDropdownList } from "../../../store/designation";
// import { getemploymentDropdown } from "../../../store/employment";
// import { getbranchlist } from "../../../store/branch";
// import { getNewPermissionRole } from "../../../store/users";
// import { Select } from "antd";
// import Select from "react-select";
// import {
//   adduserregistrationData,
//   clearUserregistration,
// } from "../../../store/userregistration";
// import { clearUserSettings } from "../../../store/usersettings";
...
export default function RegistrationPage({ nextcall, setUserId }) {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const depData = useSelector((state) => state.department.dropDownData);
  const desigData = useSelector((state) => state.designation.dropDownData);
  const emp_typeData = useSelector((state) => state.employment.dropDownData);
  const userParamRole = useSelector((state) => state.userSlice.paramRole);
  const branch_data = useSelector((state) => state.branch.branchList);
  const added_data = useSelector((state) => state.userregistration.newData);
  ...
  const selector = useSelector((state) => state.registerDetails);
  ...
  const onSubmit = (data) => {
    ...
    dispatch(userRegister(token, formData));
  };

  // get data of department, designation, emp_type, branch and userRole
  useEffect(() => {
    console.log("in empty use effect of user registration");
    reset();
    // dispatch(clearUserregistration());
    dispatch(getdeptDropdownList(token));
    dispatch(getdesigDropdownList(token));
    dispatch(getemploymentDropdown(token));
    dispatch(getbranchlist(token));
    dispatch(getNewPermissionRole(token));
    // dispatch(clearUserSettings());
  }, []);

  useEffect(() => {
    if (userParamRole) {
      setMultiDrop(userParamRole);
    }
  }, [userParamRole]);

  useEffect(() => {
    if (added_data) {
      setUserId(added_data?.id);
      nextcall();
    }
  }, [added_data]);
  ...
  const imageFormDataConverter2 = (e) => {
    // console.log("Rohhit",e.target.files[0]);
                    ...
                    <div className="input-block ">
                      ...
                      <div className="">
                        ...
                        >
                          ...
                          {branch_data?.map((dep) => {
                            return (
                              <option value={dep.id}>{dep.branch_name}</option>
                            );
                          })}
                        </select>{" "}
                        <div className="text-danger">
                          ...
                          render={({ field }) => (
                            <>
                              <input
                                ...
                                // {...field}
                                // {...register("profile_image")}
                                onChange={(e) => imageFormDataConverter(e)}
                              />
                              <div className="text-danger">
                                {errors.profile_image?.message}
                              </div>
