...
import { deleteExpensepanelAction, expensetableTrueSubmit, exportExpenseListAction, getManagerListAction, getexpensePanelListAction } from "../../store/Actions/Actions";
...
const ExpensePanel = () => {
  ...
  const expenseselector = useSelector(
    (state) => state.panelExpenseResult
  );
...
const showApproveModal = (record) => {
 ...
 function downloadExlsFiles() {
  ...
  dispatch(
    exportExpenseListAction({ URL: exportUrl })
  );
}
  ...
  return (
    ...
    },
    {
      ...
      >
        ...
        <Button
          type="primary"
          onClick={() => handelsubmittrue(selectedRecord, selectedManagerId)}
        >
          OK
        </Button>
        </Col>
