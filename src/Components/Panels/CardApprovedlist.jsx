...
import { deleteExpensepanelAction, expensetableTrueSubmit, exportExpenseListAction, getManagerListAction, getexpensePanelListAction, managercardapprovelAction } from "../../store/Actions/Actions";
...
const CardApprovedlist = () => {
  ...
  useEffect(() => {
    if (managerapprovedSelector) {
      const allExpense = managerapprovedSelector.results.map((element,index) => ({
        srno:index+1,
        id: element.id,
