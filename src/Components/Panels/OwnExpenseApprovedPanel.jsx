...
import { ownExpenseApproveAction } from "../../store/Actions/Actions";
import Addexpense from "../Screens/Dashboard/SubMenu/Addexpense";

function OwnExpenseApprovedPanel() {
  ...
  const [url, setUrl] = useState(URLS.OWN_EXPENSE_APPROVE_URL);
  const dispatch = useDispatch();
  const ownExpenseapprovedSelector = useSelector(
    (state) => state.ownExpenseApproveResult
  );

  function getPageDetails(url) {
    dispatch(ownExpenseApproveAction({ payload: {}, URL: url }));
  }

  function changePage(page, pageSize) {
    let urlNew = URLS.OWN_EXPENSE_APPROVE_URL + "?page=" + page;
    setUrl(urlNew);
  ...
  function handleSearch() {
    ...
    let urlForSearch = URLS.OWN_EXPENSE_APPROVE_URL;
  ...
  useEffect(() => {
    if (ownExpenseapprovedSelector) {
      const allExpense = ownExpenseapprovedSelector.results.map(
        (element, index) => ({
          srno: index + 1,
          id: element.id,
      ...
      pageObj.count = ownExpenseapprovedSelector.count;
      pageObj.page_size = ownExpenseapprovedSelector.page_size;
      setPaginationCount(pageObj);
    }
  }, [ownExpenseapprovedSelector]);
...
<Modal
        title={viewCompanyData ? "Approved Expenses" : "Update"}
...
export default OwnExpenseApprovedPanel;
