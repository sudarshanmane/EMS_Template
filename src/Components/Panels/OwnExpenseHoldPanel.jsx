...
import { ownExpenseHoldAction } from "../../store/Actions/Actions";
import Addexpense from "../Screens/Dashboard/SubMenu/Addexpense";

function OwnExpenseHoldPanel() {
  ...
  const [url, setUrl] = useState(URLS.OWN_EXPENSE_HOLD_URL);
  const dispatch = useDispatch();
  const ownExpenseHoldSelector = useSelector(
    (state) => state.ownExpenseHoldListResult
  );

  function getPageDetails(url) {
    dispatch(ownExpenseHoldAction({ payload: {}, URL: url }));
  }

  function changePage(page, pageSize) {
    let urlNew = URLS.OWN_EXPENSE_HOLD_URL + "?page=" + page;
    setUrl(urlNew);
  ...
  function handleSearch() {
    ...
    let urlForSearch = URLS.OWN_EXPENSE_HOLD_URL;
  ...
  useEffect(() => {
    if (ownExpenseHoldSelector) {
      const allExpense = ownExpenseHoldSelector.results.map(
        (element, index) => ({
          srno: index + 1,
          id: element.id,
      ...
      pageObj.count = ownExpenseHoldSelector.count;
      pageObj.page_size = ownExpenseHoldSelector.page_size;
      setPaginationCount(pageObj);
    }
  }, [ownExpenseHoldSelector]);
...
<Modal
        title={viewCompanyData ? "Hold Expenses" : "Update"}
...
export default OwnExpenseHoldPanel;
