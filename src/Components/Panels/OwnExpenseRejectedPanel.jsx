...
import { ownExpenseRejectAction } from "../../store/Actions/Actions";
import Addexpense from "../Screens/Dashboard/SubMenu/Addexpense";

function OwnExpenseRejectedPanel() {
  ...
  const [url, setUrl] = useState(URLS.OWN_EXPENSE_REJECT_URL);
  const dispatch = useDispatch();
  const ownExpenserejectedSelector = useSelector(
    (state) => state.ownExpenseRejectListResult
  );

  function getPageDetails(url) {
    dispatch(ownExpenseRejectAction({ payload: {}, URL: url }));
  }

  function changePage(page, pageSize) {
    let urlNew = URLS.OWN_EXPENSE_REJECT_URL + "?page=" + page;
    setUrl(urlNew);
  ...
  function handleSearch() {
    ...
    let urlForSearch = URLS.OWN_EXPENSE_REJECT_URL;
  ...
  useEffect(() => {
    if (ownExpenserejectedSelector) {
      const allExpense = ownExpenserejectedSelector.results.map(
        (element, index) => ({
          srno: index + 1,
          id: element.id,
      ...
      pageObj.count = ownExpenserejectedSelector.count;
      pageObj.page_size = ownExpenserejectedSelector.page_size;
      setPaginationCount(pageObj);
    }
  }, [ownExpenserejectedSelector]);
  ...
  return (
    <div>
      ...
      />
      <Modal
        title={viewCompanyData ? "Rejected Expenses" : "Update"}
...
export default OwnExpenseRejectedPanel;
